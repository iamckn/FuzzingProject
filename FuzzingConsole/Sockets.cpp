...
int DatagramSocket::send(const struct sockaddr* dest, const char * message, size_t length )
{
    ...
    if (retVal == -1 ){
        ...
        LOG(ERROR)<<"DatagramSocket::send() failed";
    }
	return retVal;
}
...
UDPSocket::UDPSocket(unsigned short wSrcPort)
    :DatagramSocket(),mbroadcast(true)
{
	open(wSrcPort);
}
...
int UDPSocket::broadcast(const char * msg,unsigned short wDestPort){
    struct sockaddr_in addr_bc;
    bzero(&addr_bc,sizeof(sockaddr_in));
    addr_bc.sin_family=AF_INET;
    addr_bc.sin_addr.s_addr=htonl(INADDR_BROADCAST);
    addr_bc.sin_port=htons(wDestPort);
    if(setsockopt(mSocketFD,SOL_SOCKET,SO_BROADCAST,&mbroadcast,sizeof(mbroadcast))!=0){
        LOG(ERROR)<<"setsockopt error";
        return -1;
    }
    return send((struct sockaddr*)&addr_bc,msg);
}
