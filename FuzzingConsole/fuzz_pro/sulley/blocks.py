...
class GSMblock(block):
    def str2int(self,string,endian='<'):
        t_sum=0
        count=0
        for item in list(string):
            (t,)=struct.unpack('B',item)
            if endian=='<':
                t_sum+=t*(2**(8*count))
            else:
                t_sum=(t_sum<<8)+t
    ...
    def render(self):
        ...
        for item in self.stack:
            if not isinstance(item,primitives.bit_field) or isinstance(item,primitives.GSMrepeat_bit_field):
                self.rendered+=item.rendered
            else:
                t_len+=item.width
                t_value=(t_value<<item.width)+self.str2int(item.rendered)
                (a,b)=divmod(t_len,8)
                if b==0:
                    while a>0:
                        t=struct.pack('B',t_value&0xff)
                        self.rendered+=t
                        t_value>>=8
                        a-=1
                    t_len=0
