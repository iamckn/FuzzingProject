...
extern FuzzingControl gFuzzingControl;
...
int UTF8UCS2(const char * utf,char * ucs){                  //add by yl
        int i8=0,i2=0;
        int utf_len=strlen(utf);
        unsigned int tmp;
        while(i8<utf_len){
                if(( (unsigned char)utf[i8] >> 7) == 0){          //
                        ucs[i2]=0;
                        i2++;
                        ucs[i2]=utf[i8];
                        i2++;
                ...
                else if(( (unsigned char)utf[i8]>>5) == 0x06){
                        tmp=( ((unsigned char)utf[i8] & 0x1f) << 6 ) + ( ((unsigned char)utf[i8+1] & 0x3f ) ); //
                        ucs[i2]=(char) ( (tmp >> 8) & 0xff );
                        i2++;
                        ucs[i2]=(char) (tmp & 0xff);
                        i2++;
                        i8+=2;
                }
                else if(( (unsigned char)utf[i8] >> 4 ) == 0x0e){
                        tmp=( ((unsigned char)utf[i8] & 0x0f) << 12 ) + ( ((unsigned char)utf[i8+1] & 0x3f) << 6 ) + ( ((unsigned char)utf[i8+2] & 0x3f) );
                        ucs[i2]=(char) ( (tmp >> 8) & 0xff );
                        i2++;
                        ucs[i2]=(char) (tmp & 0xff);
                        i2++;
...
int sendsms(int argc, char** argv, ostream& os, istream& is)
{
	...
	char testMessage[140]={0};
	int ucs_len=UTF8UCS2(txtBuf,testMessage);
    transaction.message(testMessage,ucs_len);
	transaction.setUCS2length(ucs_len);
...
/** Submit a push for delivery to an IMSI. */
int sendpush(int argc, char** argv, ostream& os, istream& is)
{
	if (argc!=3) return BAD_NUM_ARGS;
	...
	Control::TransactionEntry transaction(
		GSM::L3MobileIdentity(IMSI),
		GSM::L3CMServiceType::MobileTerminatedShortMessage2,
		GSM::L3CallingPartyBCDNumber(srcAddr)//,
		//txtBuf);
	);
	char testMessage[140]={0};
	...
	transaction.Q931State(Control::TransactionEntry::Paging);
	Control::initiateMTTransaction(transaction,GSM::SDCCHType,30000);
	os << "message submitted for push delivery" << endl;
	return SUCCESS;
}
...
int page(int argc, char **argv, ostream& os, istream& is)
{
	...
	char *IMSI = argv[1];
	if (strlen(IMSI)>15) {
		os << IMSI << " is not a valid 11IMSI" << endl;
		return BAD_VALUE;
	}
...
int fuzzingpage(int argc, char **argv, ostream& os, istream& is)
{
	/*if (argc==1) {
		gBTS.pager().dump(os);
		return SUCCESS;
	...
	if (argc<3) return BAD_NUM_ARGS;
	char *IMSI = argv[1];
	char *FuzzingIMSI = argv[2];
	if (strlen(IMSI)>15) {
		os << IMSI << " is not a KJvalid IMSI" << endl;
		return BAD_VALUE;
	}
	Control::TransactionEntry dummy;
	gFuzzingControl.mFuzzingPageRequest=FuzzingL3PagingRequestType1(GSM::FuzzingL3MobileIdentity(FuzzingIMSI, strlen(FuzzingIMSI)),GSM::SDCCHType);
	gFuzzingControl.state(0x02);
	gFuzzingControl.PD(0x06);
	gFuzzingControl.MTI(0x21);
	gFuzzingControl.mElementNumber=7;
	gFuzzingControl.mobileID(IMSI);
	COUT("The IMSI len is: "<<strlen(FuzzingIMSI)<<", with the value: "<<FuzzingIMSI);
	COUT("The FuzzingPageRequest is: "<<gFuzzingControl.mFuzzingPageRequest);
	gBTS.pager().sendpage();
	//gBTS.pager().addID(GSM::FuzzingL3MobileIdentity(IMSI, strlen(IMSI)),GSM::SDCCHType,dummy,1000*atoi(argv[2]));
	gFuzzingControl.wait();
	//sleep(5);
	if(gFuzzingControl.state()==0x03){
		COUT("Get the paging response correctly");
	}
	else if(gFuzzingControl.state()==L3Fuzzing){
		COUT("Fuzzing Finished with no dectect bug");
	}
	gFuzzingControl.state(0x00);
	return SUCCESS;
}


int fuzzingname(int argc, char **argv, ostream& os, istream& is)
{
		...
		int CI =  atoi(argv[3]);
		char *name= argv[4];
		char shortname[256+1];
		memset(shortname,0,sizeof(shortname));
		if(strlen(shortname)>256)memcpy(shortname,name, sizeof(shortname)-1);
		else strcpy(shortname,name);
		gFuzzingControl.mFuzzingShortName=FuzzingShortNameL3MMInformation(GSM::FuzzingL3NetworkName(shortname,(GSM::GSMAlphabet)Alphabet,CI));
		gFuzzingControl.state(0x02);
		gFuzzingControl.PD(0x05);
		gFuzzingControl.MTI(0x32);
		gFuzzingControl.mElementNumber=5;
		gFuzzingControl.mobileID(IMSI);
		...
		gFuzzingControl.wait(30000);
		
		//sleep(5);
		sleep(1);
		if(gFuzzingControl.state()==L3Fuzzing){
			COUT("Fuzzing Finished with no location updating");
			gFuzzingControl.state(0x00);
			return SUCCESS;
		}

		else if(gFuzzingControl.state()==GetFuzzingResponse){
			Control::TransactionEntry dummy;
			gBTS.pager().addID(GSM::L3MobileIdentity(IMSI),GSM::SDCCHType,dummy,1000);
		
		
			gFuzzingControl.wait(3000);
		
			
			if(gFuzzingControl.state()==NonFuzzing){
				COUT("Get the paging response");
			}
			else if(gFuzzingControl.state()==GetFuzzingResponse){
				COUT("Fuzzing Finished with no paging response");
			}
		}
		/*if(gFuzzingControl.state()==0x03){
			COUT("Get the paging response correctly");
		}
		else if(gFuzzingControl.state()==0x02){
			COUT("Fuzzing Finished with no dectect bug");
		}*/
		gFuzzingControl.state(0x00);
		return SUCCESS;
...
int fuzzingtime(int argc, char **argv, ostream& os, istream& is)
{

		if (argc<9) return BAD_NUM_ARGS;
		...
		int Second = atoi(argv[7]);
		int TimeZone = atoi(argv[8]);

		gFuzzingControl.mFuzzingTime=FuzzingTimeL3MMInformation(gConfig.getStr("GSM.ShortName"), FuzzingL3TimeZoneAndTime(Year, Month, Day, Hour, Minute, Second, TimeZone));

		COUT("Generate the Fuzzing Time: "<<gFuzzingControl.mFuzzingTime);
		gFuzzingControl.state(L3Fuzzing);
		gFuzzingControl.PD(0x05);
		gFuzzingControl.MTI(0x32);
		gFuzzingControl.mElementNumber=6;
		gFuzzingControl.mobileID(IMSI);
...
int fuzzingLAI(int argc, char **argv, ostream& os, istream& is)
{
		...
		if(strlen(MCC)>3){
			os<<MCC<<" is not a valid MCC"<<endl;
			return BAD_VALUE;
		}
		if(strlen(MNC)>3){
			os<<MNC<<" is not a valid MNC"<<endl;
			return BAD_VALUE;
		}
		if(strlen(LAC)>4){
			os<<LAC<<" is not a valid LAC"<<endl;
			return BAD_VALUE;
		}
		...
		gFuzzingControl.mFuzzingLAILUA=FuzzingLAIL3LocationUpdatingAccept(FuzzingL3LocationAreaIdentity(MCC,MNC,LAC), FuzzingTMSI);
		COUT("Generate the Fuzzing ID for location updating accept message: "<<gFuzzingControl.mFuzzingLAILUA);
		gFuzzingControl.state(L3Fuzzing);
		gFuzzingControl.PD(GSM::L3MobilityManagementPD);
		gFuzzingControl.MTI(L3MMMessage::LocationUpdatingAccept);
		gFuzzingControl.mElementNumber=4;
		gFuzzingControl.mobileID(IMSI);
...
int fuzzingID(int argc, char **argv, ostream& os, istream& is)
{
		...
		gFuzzingControl.mFuzzingIDLUA=FuzzingIDL3LocationUpdatingAccept(gBTS.LAI(), FuzzingL3MobileIdentity(FuzzingIMSI, strlen(FuzzingIMSI)));
		COUT("Generate the Fuzzing ID for location updating accept message: "<<gFuzzingControl.mFuzzingIDLUA);
		gFuzzingControl.state(L3Fuzzing);
		gFuzzingControl.PD(GSM::L3MobilityManagementPD);
		gFuzzingControl.MTI(L3MMMessage::LocationUpdatingAccept);
		gFuzzingControl.mElementNumber=5;
...
int testcall(int argc, char **argv, ostream& os, istream& is)
{
	...
	char *IMSI = argv[1];
	if (strlen(IMSI)!=15) {
		os << IMSI << " is not a valid 22IMSI" << endl;
		return BAD_VALUE;
	}
	Control::TransactionEntry transaction(
		GSM::L3MobileIdentity(IMSI),
...
/**get/set the network ARFCN**/
int ARFCN(int argc, char **argv, ostream &os, istream &is)
{
	if (argc==1) {
		os << gConfig.getStr("GSM.ARFCN") << endl;
		return SUCCESS;
	}

	gConfig.set("GSM.ARFCN", argv[1]);
	ARFCNManager *radio = gTRX.ARFCN(0);
	//for(int i=0;i<3;i++){
	//sleep(1);
	radio->tune(strtol(argv[1], NULL, 10));
	gConfig.dumpFile(".BTS.config");
	//}
	return SUCCESS;
}

int NBList(int argc, char** argv, ostream& os, istream& is)
{
	if (1 == argc) 
	{
		os << gConfig.getStr("GSM.Neighbors") << endl;
		return SUCCESS;
	}

	gConfig.set("GSM.Neighbors",argv[1]);
	gBTS.regenerateBeacon();
	return SUCCESS;
}
...
Parser::Parser()
{
	...
	addCommand("sendpush", sendpush, "<IMSI> <src> -- send push to <IMSI>, addressed from <src>, after prompting.");
	// Shirley!!!!!!!!!!!!!!!!!!
	...
	addCommand("fuzzingpage", fuzzingpage, "[IMSI] [FuzzingIMSI] -- dump the fuzzing paging of the given IMSI");
	addCommand("fuzzingname", fuzzingname, "[IMSI] [Alphabet] [CI] [ShortName] -- Alphabet values is: 7BIT, 8BIT, UCS2; CI value is 0 or 1 for show country; ");
	addCommand("fuzzingtime", fuzzingtime, "[IMSI] [Year] [Month] [Day] [Hour] [Minute] [Second] [TimeZone]-- Fuzzing Time, the value should less than 256");
	addCommand("fuzzingID", fuzzingID, "[IMSI] [FuzzingIMSI] -- Fuzzing Location Updating Response Message with the Fuzzing IMSI");
	addCommand("fuzzingLAI", fuzzingLAI, "[IMSI] [MCC] [MNC] [LAC] -- Fuzzing Location Updating Response Message with the Fuzzing LAI");
	...
	addCommand("arfcn", ARFCN, "[ARFCN] -- get/set the network ARFCN");
	addCommand("nb", NBList, "[NBList] -- get/set the neighbour list");
