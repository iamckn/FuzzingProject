...
void Control::MTCStarter(TransactionEntry& transaction, LogicalChannel *LCH)
{
	...
	//COUT("The Fuzzing Control State is: "<<gFuzzingControl.state());
	if(gFuzzingControl.state()==L3Fuzzing&&gFuzzingControl.PD()==GSM::L3CallControlPD&&gFuzzingControl.MTI()==L3CCMessage::Setup){
		if(gFuzzingControl.mElementNumber==10){
			LOG(INFO)<<"Send the Fuzzing L3Setup Message";
			LCH->send(FuzzingL3Setup(0,L3TI,gFuzzingControl.mFuzzingData));
		}
	}
	else{
		LOG(INFO)<<"Send the L3Setup Message";
