...
GSM::L3Message* getMessage(GSM::LogicalChannel* LCH, unsigned timeout_ms, unsigned SAPI=0);//added by syb
/**
...
bool deliverSMSToMS(const char *callingPartyDigits, const char* message, unsigned ucs2_len, unsigned TI, GSM::LogicalChannel *LCH);

//Shirley!!!!!!!!!!!!!!!!!!!!!
bool deliverSMSToMS2(const char *callingPartyDigits, const char* message, unsigned TI, GSM::LogicalChannel *LCH);
bool deliverSMSToMS2(const char *callingPartyDigits, const char* message, unsigned ucs2_len, unsigned TI, GSM::LogicalChannel *LCH);

bool deliverFuzzingSMS(BitVector * FuzzingBit, GSM::LogicalChannel *LCH);
...
//Shirley!!!!!!!!!!!!!!!!!!!!!
void MTSMSController2(TransactionEntry& transaction, 
						GSM::LogicalChannel *LCH);
...
class PagingEntry {
	...
	GSM::FuzzingL3MobileIdentity mFuzzingID;
	...
	*/
	...
	PagingEntry(const GSM::FuzzingL3MobileIdentity& wID, GSM::ChannelType wType,
				unsigned wTransactionID, unsigned wLife)
			:mFuzzingID(wID),mType(wType),mTransactionID(wTransactionID),mExpiration(wLife)
		{}

	/** Access the ID. */
	const GSM::L3MobileIdentity& ID() const { return mID; }
	
	const GSM::FuzzingL3MobileIdentity& FuzzingID() const { return mFuzzingID; }
...
class Pager {
	...
	*/
	...
	void addID(
		const GSM::FuzzingL3MobileIdentity& addID,
		GSM::ChannelType chanType,
		TransactionEntry& transaction,
		unsigned wLife=2*gConfig.getNum("SIP.Timer.A")
	);
	...
	*/
	...
	unsigned removeID(const GSM::FuzzingL3MobileIdentity&);
...
class TransactionEntry {
	...
	unsigned mUCS2length;				    ///< added by syb
	
	BitVector * mFuzzingCallingNumberBit;
	...
	BitVector *FuzzingBit() {return mFuzzingCallingNumberBit;}
	...
	void setUCS2length(unsigned wlen) { mUCS2length=wlen ; } //add by syb
	unsigned getUCS2length() const { return mUCS2length; }
