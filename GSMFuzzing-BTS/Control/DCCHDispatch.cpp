...
void DCCHDispatchMM(const L3MMMessage* req, LogicalChannel *DCCH)
{
	...
	switch (MTI) {
		...
		case L3MMMessage::CMServiceRequest:
			...
			break;
		default:
			COUT("unhandled MM message " << MTI << " on " << DCCH->type());
...
void DCCHDispatchRR(const L3RRMessage* req, LogicalChannel *DCCH)
{
	...
	switch (MTI) {
		...
		case L3RRMessage::AssignmentComplete:
			...
			break;
		default:
			COUT("unhandled RR message " << MTI << " on " << DCCH->type());
...
void Control::DCCHDispatcher(LogicalChannel *DCCH)
{
	while (1) {
		try {
			...
			COUT( "received " << message->PD() <<", "<<message->MTI()<<"\n"<<*message);
			...
			switch (message->PD()) {
				...
				case L3RadioResourcePD:
					...
					break;
				default:
					COUT( "unhandled protocol " << message->PD() << " on " << DCCH->type());
		...
		// Catch the various error cases.
			...
			COUT("ChannelReadTimeout");
			gFuzzingControl.state(NoResponse);
			...
			// Cause 0x03 means "abnormal release, timer expired".
			DCCH->send(L3ChannelRelease(0x03));
			COUT("Uncaught SIPTimeout, will leave a stray transcation");
			gFuzzingControl.state(FuzzingFailed);
			gFuzzingControl.signal();
		}
		catch (SIP::SIPError) {
			...
			COUT("Uncaught SIPError, will leave a stray transcation");
			gFuzzingControl.state(FuzzingFailed);
			gFuzzingControl.signal();
		}
