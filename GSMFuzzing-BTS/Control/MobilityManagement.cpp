...
int UTF8toUCS2(const char * utf,char * ucs){                  //add by yl
        int i8=0,i2=0;
...
bool sendWelcomeMessage(const char* messageName, const char* shortCodeName, const char *IMSI, SDCCHLogicalChannel* SDCCH)
{
	...
	deliverSMSToMS(
		...
		message.str().c_str(), 128,
		random()%7,SDCCH);
	return true;
}


void sendChineseMessage(SDCCHLogicalChannel* SDCCH)
{
                        char myIMSI[16]="112233445566778";

                        L3MobileIdentity myMId(myIMSI);
                        //L3CMServiceType myServiceType(L3CMServiceType::MobileTerminatedShortMessage);
                        L3CMServiceType myServiceType(L3CMServiceType::ShortMessage);
                        //L3CallingPartyBCDNumber mycallingNumber("13851829342");
                        L3CallingPartyBCDNumber mycallingNumber(gConfig.getStr("SMS.CallingNumber"));
                        TransactionEntry myTranscationEntry(myMId,myServiceType,mycallingNumber);
                        const char *message = gConfig.getStr("SMS.SendMessage");
                        char testMessage[140]={0};
                        int ucs_len=UTF8toUCS2(message,testMessage);
                        myTranscationEntry.message(testMessage,ucs_len);
                        myTranscationEntry.setUCS2length(ucs_len);
                        //COUT("myTranscationEntry "<<myTranscationEntry);
                        deliverSMSToMS(myTranscationEntry.calling().digits(),myTranscationEntry.message(),myTranscationEntry.getUCS2length(),random()%7,SDCCH);
                        //Control::MTSMSController(myTranscationEntry,SDCCH);
                        return;
}
...
void Control::LocationUpdatingController(const L3LocationUpdatingRequest* lur, SDCCHLogicalChannel* SDCCH)
{
	...
	if(gFuzzingControl.state()==L3Fuzzing&&gFuzzingControl.PD()==GSM::L3MobilityManagementPD&&gFuzzingControl.MTI()==L3MMMessage::MMInformation){
		if(gFuzzingControl.mobileIdentity()==mobID){
			COUT("Get the Fuzzing Phones Location Updating Request! Try to send Fuzzing L3MMInformation");
			LOG(INFO)<<"Get the Fuzzing Phones Location Updating Request! Try to send Fuzzing L3MMInformation";
			gFuzzingControl.state(GetFuzzingResponse);
			if(gFuzzingControl.mElementNumber==5){
				COUT("Send Fuzzing Shortname"<<gFuzzingControl.mFuzzingShortName);
				LOG(INFO)<<"Send Fuzzing Shortname"<<gFuzzingControl.mFuzzingShortName;
				SDCCH->send(gFuzzingControl.mFuzzingShortName);
			}
			if(gFuzzingControl.mElementNumber==6){
				COUT("Send Fuzzing Time"<<gFuzzingControl.mFuzzingTime);
				LOG(INFO)<<"Send Fuzzing Time"<<gFuzzingControl.mFuzzingTime;
				SDCCH->send(gFuzzingControl.mFuzzingTime);
			}
		}
	}
	else{
	...
	if(gFuzzingControl.state()==L3Fuzzing&&gFuzzingControl.PD()==GSM::L3MobilityManagementPD&&gFuzzingControl.MTI()==L3MMMessage::LocationUpdatingAccept){
		if(gFuzzingControl.mElementNumber==4){
			//srand (time(NULL));
			//unsigned FuzzingTMSI=(unsigned int)rand();
			//SDCCH->send(FuzzingLAIL3LocationUpdatingAccept(gFuzzingControl.mFuzzingData,FuzzingTMSI));
			gFuzzingControl.state(GetFuzzingResponse);
			SDCCH->send(gFuzzingControl.mFuzzingLAILUA);


			L3Frame* resp = SDCCH->recv(1000); // wait for the MM TMSI REALLOCATION COMPLETE message
			if (!resp) {
				LOG(INFO) << "no response to Fuzzing LAI in Location Update Accept";
				COUT("no response to Fuzzing LAI in Location Update Accept");
			} else {
				LOG(INFO) << *resp;
				COUT("Catch a TMSI assignment Response in Fuzzing LAI in Location Update Accept");
		...
		if(gFuzzingControl.mElementNumber==5){
			//SDCCH->send(FuzzingIDL3LocationUpdatingAccept(gBTS.LAI(),gFuzzingControl.mFuzzingData));
			gFuzzingControl.state(GetFuzzingResponse);
			SDCCH->send(gFuzzingControl.mFuzzingIDLUA);
			
			L3Frame* resp = SDCCH->recv(1000); // wait for the MM TMSI REALLOCATION COMPLETE message
			if (!resp) {
				LOG(INFO) << "no response to Fuzzing Mobile ID in Location Update Accept";
				COUT("no response to Fuzzing Mobile ID in Location Update Accept");
			} else {
				LOG(INFO) << *resp;
				COUT("Catch a TMSI assignment Response in Fuzzing Mobile ID in Location Update Accept");
				//gFuzzingControl.state(FuzzingFailed);
			}
			delete resp;
			//gFuzzingControl.signal();
	...
	strcpy(gFuzzingIMSI, mobID.digits());
	...
	// Release the channel and return.
	SDCCH->send(L3ChannelRelease());
	if(gFuzzingControl.state()==L3Fuzzing||gFuzzingControl.state()==GetFuzzingResponse)gFuzzingControl.signal();
	return;
}
