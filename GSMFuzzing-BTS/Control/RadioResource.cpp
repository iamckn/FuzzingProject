...
unsigned Pager::removeID(const FuzzingL3MobileIdentity& delID)
{
	// Return the associated transaction ID, or 0 if none found.
	unsigned retVal = 0;
	LOG(INFO) << delID;
...
{
	if(gFuzzingControl.state()==L3Fuzzing&&gFuzzingControl.PD()==GSM::L3RadioResourcePD&&gFuzzingControl.MTI()==L3RRMessage::PagingRequestType1){
		//LOG(INFO)<<"Sending Fuzzing L3Paging RequestType1 "<<FuzzingL3PagingRequestType1(FuzzingID1,type1);
		LOG(INFO)<<"Sending Fuzzing L3Paging RequestType1 ";
		COUT("Sending Fuzzing L3Paging RequestType1 ");
		//gBTS.getPCH(0)->send(FuzzingL3PagingRequestType1(FuzzingID1,type1));
		//gBTS.getPCH(0)->send(FuzzingL3PagingRequestType1(FuzzingID1,type1));
		//gBTS.getPCH(0)->send(FuzzingL3PagingRequestType1(FuzzingID1,type1), DATA);
		//gBTS.getPCH(0)->send(FuzzingL3PagingRequestType1(FuzzingID1,type1), DATA);
		//gBTS.getPCH(0)->send(gFuzzingControl.mFuzzingPageRequest);
		//gBTS.getPCH(0)->send(gFuzzingControl.mFuzzingPageRequest);
		if(gFuzzingControl.mL2FrameType==0){
			gBTS.getPCH(0)->send(gFuzzingControl.mFuzzingPageRequest,UNIT_DATA);
			gBTS.getPCH(0)->send(gFuzzingControl.mFuzzingPageRequest,UNIT_DATA);
		}
		else if(gFuzzingControl.mL2FrameType==1){
			gBTS.getPCH(0)->send(gFuzzingControl.mFuzzingPageRequest,DATA);
			gBTS.getPCH(0)->send(gFuzzingControl.mFuzzingPageRequest,DATA);
		}
		//removeID(FuzzingID1);
	}
