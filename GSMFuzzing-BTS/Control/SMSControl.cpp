...
bool Control::deliverSMSToMS(const char *callingPartyDigits, const char* message, unsigned  ucs2_len, unsigned TI, LogicalChannel *LCH)
{
	...
	unsigned reference = random() % 255;
        ...
        //************added by syb************
        TLUserData ud(message,ucs2_len);
        ud.DCS(8);          //Chinese
        TLDeliver dl(callingPartyDigits,ud);
        //*************************************

	CPData deliver(0,TI,
		RPData(reference,
		RPAddress(InternationalNumber,gConfig.getStr("SMS.FakeSrcSMSC")),
		//RPAddress(gConfig.getStr("SMS.FakeSrcSMSC")),
                        dl));   //TLDeliver(callingPartyDigits,message,TLPID)));
	LOG(WARN) << "sending " << deliver;
	//COUT("\nsending: "<<deliver<<"\n");
	//COUT(deliver.RPDU()<<"\n");
	LCH->send(deliver,3);
...
bool Control::deliverSMSToMS2(const char *callingPartyDigits, const char* message, unsigned  ucs2_len, unsigned TI, LogicalChannel *LCH)
{
	// This function is used to deliver wap push messages that originate INSIDE the BTS.
	// For the normal SMS delivery, see MTSMSController2.
	...
	unsigned reference = random() % 255;
        ...
        //************added by syb************
        TLUserData2 ud(message,ucs2_len);
        ud.DCS(8);          //Chinese
        TLDeliver2 dl(callingPartyDigits,ud);
        //*************************************
...
bool Control::deliverFuzzingSMS(BitVector * FuzzingBit, GSM::LogicalChannel *LCH)
{


	unsigned TI=random()%7;
	bool success = true;
	...
	//for Fuzzing RP-Data
	//FuzzingCPData deliver(0, TI, FuzzingBit);
	...
	CPData deliver(0,TI,
		RPData(reference,
		RPAddress(InternationalNumber,gConfig.getStr("SMS.FakeSrcSMSC")),
        RPUserData(TLFrame(*FuzzingBit))));        //!!!!!!!!!!!!!!!!!!!!useful
	...
	if (CM->MTI()!=CPMessage::ACK) {
		LOG(WARN) << "MS rejected our RP-DATA with CP message with TI=" << CM->MTI();
		throw UnexpectedMessage();
		//LCH->send(L3ChannelRelease(0x62));
		//success=false;
		//return success;
	}
	...
	try {
		...
		LOG(WARN) << "SMS parsing failed (in L3)";
		throw UnsupportedMessage();
		//LCH->send(L3ChannelRelease(0x61));
		//success=false;
		//return success;
	}
						...
						LogicalChannel *LCH)
{
    ...
    bool success = deliverSMSToMS(transaction.calling().digits(),transaction.message(),transaction.getUCS2length(),random()%7,LCH);
...
//Shirley!!!!!!!!!!!!!!!!!!!!!!!!!!
void Control::MTSMSController2(TransactionEntry& transaction, 
						LogicalChannel *LCH)
{
	assert(LCH);
    ...
    bool success = deliverSMSToMS2(transaction.calling().digits(),transaction.message(),transaction.getUCS2length(),random()%7,LCH);
