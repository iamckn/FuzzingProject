...
void GSMConfig::regenerateBeacon()
{
	...
	LOG(INFO) << SI3;
	SI3.write(l3);
	L2Header SI3Header(L2Length(18));//l3.length()));
	...
	LOG(INFO) << SI4;
	SI4.write(l3);
	L2Header SI4Header(L2Length(12));//l3.length()));
...
/*void GSMConfig::regenerateFuzzingBeacon(int wMTI, L3Frame * wFrame)
{

	switch((L3RRMessage::MessageType)wMTI){
		case L3RRMessage::SystemInformationType1:
			L2Header SI1Header(L2Length(wFrame.length()));
			mSI1Frame=L2Frame(SI1Header, wFrame);
			LOG(DEBUG)<<"Fuzzing mSI1Frame is"<<mSI1Frame;
			break;
		case L3RRMessage::SystemInformationType2:
			L2Header SI2Header(L2Length(wFrame.length()));
			mSI2Frame=L2Frame(SI2Header, wFrame);
			LOG(DEBUG)<<"Fuzzing mSI2Frame is"<<mSI2Frame;
			break;
		case L3RRMessage::SystemInformationType3:
			L2Header SI3Header(L2Length(18));
			mSI3Frame=L2Frame(SI3Header, wFrame);
			LOG(DEBUG)<<"Fuzzing mSI3Frame is"<<mSI3Frame;
			break;
		case L3RRMessage::SystemInformationType4:			
			L2Header SI4Header(L2Length(12));
			mSI4Frame=L2Frame(SI4Header, wFrame);
			LOG(DEBUG)<<"Fuzzing mSI4Frame is"<<mSI4Frame;
			break;
		case L3RRMessage::SystemInformationType5:			
			mSI5Frame=wFrame);
			LOG(DEBUG)<<"Fuzzing mSI5Frame is"<<mSI5Frame;
			break;
		case L3RRMessage::SystemInformationType6:			
			mSI6Frame=wFrame;
			LOG(DEBUG)<<"Fuzzing mSI6Frame is"<<mSI6Frame;
			break;
		}
