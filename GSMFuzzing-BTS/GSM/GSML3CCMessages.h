...
/**
	...
	but the TLV format is flexiable enough to allow us to use one class for both.
*/
class FuzzingL3Setup : public L3CCMessage
{
	...
	BitVector *mFuzzingCallingNumber;
	//FuzzingL3CallingPartyBCDNumber mFuzzingCNumber;


public:

	FuzzingL3Setup(unsigned wTIFlag=0, unsigned wTIValue=7)
		:L3CCMessage(wTIFlag,wTIValue),
		mHaveBearerCapability(false),
		mHaveCallingPartyBCDNumber(false),
		mHaveCalledPartyBCDNumber(false)
	...
	FuzzingL3Setup(unsigned wTIFlag, unsigned wTIValue, BitVector * wFuzzingCallingNumber)
		:L3CCMessage(wTIFlag,wTIValue), 
		mHaveBearerCapability(false),
		mHaveCallingPartyBCDNumber(true),mFuzzingCallingNumber(wFuzzingCallingNumber),
		mHaveCalledPartyBCDNumber(false)
	{ }
