...
FuzzingL3LocationAreaIdentity::FuzzingL3LocationAreaIdentity(const char*wMCC, const char* wMNC, const char* wLAC)
	:L3ProtocolElement()
{
	// country code
	assert(strlen(wMCC)<=3);
	char * p=NULL;
	mMCC=strtoul(wMCC, &p, 16);
	
	// network code
	assert(strlen(wMNC)<=3);
	mMNC=strtoul(wMNC, &p, 16);

	assert(strlen(wLAC)<=4);
	mLAC=strtoul(wLAC, &p, 16);
}


void FuzzingL3LocationAreaIdentity::writeV(L3Frame& dest, size_t &wp) const
{
	/*dest.writeField(wp,mMCC[1],4);
	dest.writeField(wp,mMCC[0],4);
	dest.writeField(wp,mMNC[2],4);
	...
	dest.writeField(wp, (mMCC>>4)&0x0f, 4);
	dest.writeField(wp, mMCC&0x0f, 4);
	dest.writeField(wp, mMNC>>8, 4);
	dest.writeField(wp, mMCC>>8, 4);
	dest.writeField(wp, (mMNC>>4)&0x0f, 4);
	dest.writeField(wp, mMNC&0x0f, 4);
	dest.writeField(wp,mLAC,16);
...
bool FuzzingL3LocationAreaIdentity::operator==(const FuzzingL3LocationAreaIdentity& other) const
{
	if(mMCC!=other.mMCC) return false;
	if(mMNC!=other.mMNC) return false;
	if(mLAC!=other.mLAC) return false;
	return true;
}


void FuzzingL3LocationAreaIdentity::text(ostream& os) const
{
	os << "MCC=0x" << std::hex<<mMCC;
	os << " MNC=0x" <<std::hex<< mMNC;
	os << " LAC=0x" << std::hex << mLAC<<endl;
}
...
bool FuzzingL3MobileIdentity::operator==(const FuzzingL3MobileIdentity& other) const
{
	if (other.mType!=mType) return false;
	if (mType==TMSIType) return (mTMSI==other.mTMSI);
	return (memcmp(mDigits,other.mDigits,mDigitsLen)==0);
}
...
size_t FuzzingL3MobileIdentity::lengthV() const
{
	if (mType==NoIDType) return 1;
	if (mType==TMSIType) return 5;
	if(mFuzzingBitEnable){ return mFuzzingData->size()/8;}
	else
		return 1+mDigitsLen/2; // 1 + strlen(mDigits)/2;
}


void FuzzingL3MobileIdentity::writeV(L3Frame& dest, size_t &wp) const
{
	// See GSM 04.08 10.5.1.4.
	...
	if(mFuzzingBitEnable){
		unsigned numBits = mFuzzingData->size();
		mFuzzingData->copyToSegment(dest,wp,numBits);
		wp += numBits;
	}
	else{

		int numDigits = mDigitsLen;//strlen(mDigits);
		COUT("numDigits is "<<mDigitsLen); 
...
void FuzzingL3MobileIdentity::parseV( const L3Frame& src, size_t &rp, size_t expectedLength)
{
	// See GSM 04.08 10.5.1.4.
...
void FuzzingL3MobileIdentity::text(ostream& os) const
{
	os << mType << "=";
	if (mType==TMSIType) {
		os << hex << "0x" << mTMSI << dec;
