...
/** LAI, GSM 04.08 10.5.1.3 */
class FuzzingL3LocationAreaIdentity : public L3ProtocolElement {

	private:

	unsigned mMCC;		///< mobile country code digits
	unsigned  mMNC;		///< mobile network code digits
	unsigned mLAC;			///< location area code
	...
	/**
		...
		@param wLAC LAC as a number.
	*/
	FuzzingL3LocationAreaIdentity(
		const char*wMCC = gConfig.getStr("GSM.MCC"),
		const char* wMNC = gConfig.getStr("GSM.MNC"),
		const char* wLAC = gConfig.getStr("GSM.LAC")
	);

	/** Sometimes we need to compare these things. */
	bool operator==(const FuzzingL3LocationAreaIdentity&) const;

	size_t lengthV() const { return 5; }
	void writeV(L3Frame& dest, size_t &wp) const;
	void parseV(const L3Frame& source, size_t &rp) {abort();}
	void parseV(const L3Frame&, size_t&, size_t) { abort(); }
	void text(std::ostream&) const;
};
...
class FuzzingL3MobileIdentity : public L3ProtocolElement {
	...
	bool mFuzzingBitEnable;

	public:

	/** Empty ID */
	FuzzingL3MobileIdentity()
		:L3ProtocolElement(),
		mType(NoIDType)
	{ mDigits[0]='\0'; } 

	/** TMSI initializer. */
	FuzzingL3MobileIdentity(unsigned int wTMSI)
		:L3ProtocolElement(),
		mType(TMSIType), mTMSI(wTMSI)
	{ mDigits[0]='\0'; } 

	/** IMSI initializer. */
	FuzzingL3MobileIdentity(const char* wDigits, size_t wDigitsLen=256)
		:L3ProtocolElement(),
		mType(IMSIType), mFuzzingBitEnable(false)
	{ memset(mDigits,0x00, 256);if(wDigitsLen>256)mDigitsLen=256; else mDigitsLen=wDigitsLen; memcpy(mDigits,wDigits,wDigitsLen); }

	FuzzingL3MobileIdentity(BitVector *wFuzzingData)
		:mFuzzingData(wFuzzingData),mFuzzingBitEnable(true)
	{
	...
	const char* digits() const { assert(mType!=TMSIType); return mDigits; }
	unsigned int TMSI() const { assert(mType==TMSIType); return mTMSI; }
	bool FuzzingBitEnabled() const {return mFuzzingBitEnable;}
	//@}

	/** Comparison. */
	bool operator==(const FuzzingL3MobileIdentity&) const;
