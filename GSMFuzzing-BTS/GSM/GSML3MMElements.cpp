...
ostream& GSM::operator<<(ostream& os, L3CMServiceType::TypeCode code)
{
	switch (code) {
		...
		///Shirley!!!!!!!!!!!!!!
		case L3CMServiceType::MobileTerminatedShortMessage2: os << "MTSMS2"; break;
...
void FuzzingL3NetworkName::writeV(L3Frame& dest, size_t &wp) const
{
	unsigned sz = strlen(mName);
	// header byte
	if (mAlphabet == ALPHABET_UCS2) {
...
void FuzzingL3NetworkName::text(std::ostream& os) const
{
	os << mName;
}
...
void FuzzingL3TimeZoneAndTime::writeV(L3Frame& dest, size_t& wp) const
{
	dest.writeField(wp, mYear%256, 8);
	dest.writeField(wp, mMonth%256, 8);
	dest.writeField(wp, mDay%256, 8);
	dest.writeField(wp, mHour%256, 8);
	dest.writeField(wp, mMinute%256, 8);
	dest.writeField(wp, mSecond%256, 8);
	dest.writeField(wp, mTime_zone%256, 8);
}

void FuzzingL3TimeZoneAndTime::text(ostream& os) const
{
	
	os << "year=" << mYear << " month=" << mMonth << " day=" << mDay
	   << " hour=" << mHour<< " min=" << mMinute<< " sec=" << mSecond
	           << " zone=" << mTime_zone;
}


void L3LocationUpdatingType::parseV(const L3Frame& src, size_t &rp)
{
	mType = (TypeCode)src.readField(rp,3);
	FOR=(int)src.readField(rp,1);
}


ostream& GSM::operator<<(ostream& os, L3LocationUpdatingType::TypeCode code)
{
	switch (code) {
		case L3LocationUpdatingType::Normal: os << "Normal Location Updating"; break;
		case L3LocationUpdatingType::Periodic: os << "Periodic Location Updating"; break;
		case L3LocationUpdatingType::IMSIAttach: os << "IMSI Attach"; break;
		case L3LocationUpdatingType::Reserved: os << "Unkonw"; break;
		default: os << "?" << (int)code << "?";
	}
	return os;
}

void L3LocationUpdatingType::text(ostream& os) const
{
	os << mType;
}
// vim: ts=4 sw=4
