...
size_t FuzzingLAIL3LocationUpdatingAccept::bodyLength() const
{
	if (mHaveMobileIdentity) return mLAI.lengthV() + mMobileIdentity.lengthTLV();
	else return mLAI.lengthV();
}

void FuzzingLAIL3LocationUpdatingAccept::writeBody( L3Frame &dest, size_t &wp ) const
{
	//mLAI.writeV(dest, wp);
	if(mBitEnabled){
		unsigned numBits = mFuzzingLAI->size();
		assert(numBits==5*8);
		mFuzzingLAI->copyToSegment(dest,wp,numBits);
		wp += numBits;
	}
	else{
		mLAI.writeV(dest, wp);
...
void FuzzingLAIL3LocationUpdatingAccept::text(ostream& os) const
{
	L3MMMessage::text(os);
	if(mBitEnabled) 	os<<"FuzzingLAI=("<<mFuzzingLAI<<")";
	else
		os<<"LAI=("<<mLAI<<")";
	if (mHaveMobileIdentity) os << "ID=(" << mMobileIdentity << ")";
}
...
size_t FuzzingIDL3LocationUpdatingAccept::bodyLength() const
{
	int len=0;
	len += mLAI.lengthV();
	if (mHaveMobileIdentity) {
		if(mBitEnabled) len += mFuzzingMobileID->size()/8;
		else len+=mMobileIdentity.lengthTLV();
	}
	 return len;
}

void FuzzingIDL3LocationUpdatingAccept::writeBody( L3Frame &dest, size_t &wp ) const
{
	
	mLAI.writeV(dest, wp);
	if (mHaveMobileIdentity) {
		if(mBitEnabled){
			unsigned numBits = mFuzzingMobileID->size();
			mFuzzingMobileID->copyToSegment(dest,wp,numBits);
			}
		else mMobileIdentity.writeTLV(0x17,dest,wp);
	}
}

void FuzzingIDL3LocationUpdatingAccept::text(ostream& os) const
{
	L3MMMessage::text(os);
	os<<"LAI=("<<mLAI<<")";
	if(mBitEnabled) os<<"ID=("<<mFuzzingMobileID<<")";
	else os<<"ID=("<< mMobileIdentity << ")";
...
void L3Abort::writeBody(L3Frame& dest, size_t &wp) const
{
	mCause.writeV(dest,wp);
}

void L3Abort::text(ostream& os) const
{
	L3MMMessage::text(os);
	os << "cause=" << mCause;
}
...
size_t FuzzingShortNameL3MMInformation::bodyLength() const
{
	size_t len=0;
	//if (mShortName.lengthV()>1) len += mShortName.lengthTLV();
	if(mBitEnabled)
		len += mFuzzingShortName->size()/8;
...
void FuzzingShortNameL3MMInformation::writeBody(L3Frame &dest, size_t &wp) const
{
	//if (mShortName.lengthV()>1) mShortName.writeTLV(0x45,dest,wp);
	if(mBitEnabled){
		unsigned numBits = mFuzzingShortName->size();
		mFuzzingShortName->copyToSegment(dest,wp,numBits);
		wp += numBits;
		}
	else
...
void FuzzingShortNameL3MMInformation::text(ostream& os) const
{
	L3MMMessage::text(os);
	if(!mBitEnabled)
		os << "short name=(" << mShortName << ")";
...
size_t FuzzingTimeL3MMInformation::bodyLength() const
{
	size_t len=0;
	if (mShortName.lengthV()>1) len += mShortName.lengthTLV();
	//len += mTime.lengthTV();
	if(mBitEnabled)
		len += mFuzzingTime->size()/8;
	else
		len += mTime.lengthTV();
	return len;
}
...
void FuzzingTimeL3MMInformation::writeBody(L3Frame &dest, size_t &wp) const
{
	if (mShortName.lengthV()>1) mShortName.writeTLV(0x45,dest,wp);
	//mTime.writeTV(0x47,dest,wp);
	if(mBitEnabled){
		unsigned numBits = mFuzzingTime->size();
		mFuzzingTime->copyToSegment(dest,wp,numBits);
		}
	else		
		mTime.writeTV(0x47,dest,wp);
}
...
void FuzzingTimeL3MMInformation::text(ostream& os) const
{
	L3MMMessage::text(os);
	os << "short name=(" << mShortName << ")";
	if(!mBitEnabled)
