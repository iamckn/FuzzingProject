...
public:
	...
	const L3LocationAreaIdentity& LAI() const
		{ return mLAI; }
	const L3LocationUpdatingType& type() const
		{return mType;}
...
class FuzzingLAIL3LocationUpdatingAccept : public L3MMMessage
{
	// LAI = (V) length of 6
	FuzzingL3LocationAreaIdentity mLAI;
	bool mHaveMobileIdentity;
	L3MobileIdentity mMobileIdentity;
	BitVector *mFuzzingLAI;
	bool mBitEnabled;

	
public:

	FuzzingLAIL3LocationUpdatingAccept()
		:L3MMMessage(),
		mHaveMobileIdentity(false)
	{}

	FuzzingLAIL3LocationUpdatingAccept(
		BitVector * wFuzzingLAI,
		const L3MobileIdentity& wMobileIdentity)
		:L3MMMessage(),mMobileIdentity(wMobileIdentity),
		mHaveMobileIdentity(true),
		mFuzzingLAI(wFuzzingLAI), mBitEnabled(true)
	{}

	FuzzingLAIL3LocationUpdatingAccept(
		const FuzzingL3LocationAreaIdentity& wLAI,
		const L3MobileIdentity& wMobileIdentity)
		:L3MMMessage(), mLAI(wLAI),
		mMobileIdentity(wMobileIdentity),
		mHaveMobileIdentity(true), mBitEnabled(false)
	{}
...
class FuzzingIDL3LocationUpdatingAccept : public L3MMMessage
{
	// LAI = (V) length of 6
	L3LocationAreaIdentity mLAI;
	bool mHaveMobileIdentity;
	FuzzingL3MobileIdentity mMobileIdentity;
	BitVector *mFuzzingMobileID;
	bool mBitEnabled;

	
public:
	
	FuzzingIDL3LocationUpdatingAccept()
		:L3MMMessage(),
		mHaveMobileIdentity(false), mBitEnabled(false)
	{}
	
	FuzzingIDL3LocationUpdatingAccept(
		const L3LocationAreaIdentity& wLAI,
		BitVector * wFuzzingMobileID)
		:L3MMMessage(), 
		mLAI(wLAI), mFuzzingMobileID(wFuzzingMobileID),
		mHaveMobileIdentity(true), mBitEnabled(true)
		
	{}

	FuzzingIDL3LocationUpdatingAccept(
		const L3LocationAreaIdentity& wLAI,
		FuzzingL3MobileIdentity wFuzzingMobileID)
		:L3MMMessage(), 
		mLAI(wLAI),mMobileIdentity(wFuzzingMobileID),
		mHaveMobileIdentity(true), mBitEnabled(false)
...
/** GSM 04.08 9.2.8 */
class L3Abort : public L3MMMessage {
	...
	L3Abort(const L3RejectCause& wCause)
		:L3MMMessage(),
		mCause(wCause)
	{}

	int MTI() const { return (int)Abort; }
...
/** GSM 04.08 9.2.15a */
class FuzzingShortNameL3MMInformation : public L3MMMessage {

	private:

	FuzzingL3NetworkName mShortName;
	L3TimeZoneAndTime mTime;
	//FuzzingL3TimeZoneAndTime mTime;
	BitVector *mFuzzingShortName;
	bool mBitEnabled;
	...
	/**
		...
		@param wShortName Abbreviated network name.
	*/
	FuzzingShortNameL3MMInformation(BitVector *wFuzzingShortName=NULL, const L3TimeZoneAndTime& wTime=L3TimeZoneAndTime())
		:L3MMMessage(), mShortName("Fuzzing-Test"),
		mFuzzingShortName(wFuzzingShortName), mTime(wTime), mBitEnabled(true)
	{
		mTime.type(L3TimeZoneAndTime::UTC_TIME);
	}
	...
	FuzzingShortNameL3MMInformation(const FuzzingL3NetworkName& wShortName, const L3TimeZoneAndTime& wTime=L3TimeZoneAndTime())
		:L3MMMessage(),
		mShortName(wShortName), mTime(wTime), mBitEnabled(false)
	{
		mTime.type(L3TimeZoneAndTime::UTC_TIME);
	}
...
/** GSM 04.08 9.2.15a */
class FuzzingTimeL3MMInformation : public L3MMMessage {
	...
	/**
		...
		@param wShortName Abbreviated network name.
	*/
	FuzzingTimeL3MMInformation(const L3NetworkName& wShortName="Fuzzing BTS", BitVector * wFuzzingTime=NULL)
		:L3MMMessage(),
		mShortName(wShortName), mFuzzingTime(wFuzzingTime), mBitEnabled(true)
	{
	}

	
	FuzzingTimeL3MMInformation(const L3NetworkName& wShortName, const FuzzingL3TimeZoneAndTime& wTime=FuzzingL3TimeZoneAndTime())
		:L3MMMessage(),
		mShortName(wShortName), mTime(wTime), mBitEnabled(false)
	{
		//mTime.type(L3TimeZoneAndTime::UTC_TIME);
