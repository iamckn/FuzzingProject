...
size_t FuzzingL3PagingRequestType1::bodyLength() const
{
	size_t sum=1;
	if(mBitEnabled){
		sum +=mFuzzingData->size()/8;
	}
...
void FuzzingL3PagingRequestType1::writeBody(L3Frame& dest, size_t &wp) const
{
	// See GSM 04.08 9.1.22.
	// Page Mode Page Mode M V 1/2 10.5.2.26    
	// Channels Needed  M V 1/2 
	...
	//if(mMobileIDs[0].FuzzingBitEnabled()==true){
	//	mMobileIDs[0].writeV(dest,wp);
	//	}
	//else{
		mMobileIDs[0].writeLV(dest,wp);
	}
...
void FuzzingL3PagingRequestType1::text(ostream& os) const
{
	L3RRMessage::text(os);
	if(!mBitEnabled){
		os << " mobileIDs=(";
...
//added for C2>C1 
	unsigned int CellReSelectOffset;
	CellReSelectOffset = gConfig.getNum("Control.CELL_RESELECT_OFFSET");
	if(CellReSelectOffset>63) CellReSelectOffset=63;
	
	dest.writeField(wp,0x80 |(CellReSelectOffset & 0x3f),8);
	//dest.writeField(wp,0x9f,8);
	dest.writeField(wp,0x00,8);
	dest.writeField(wp,0x2b2b,16);
}
