...
/** Paging Request Type 1, GSM 04.08 9.1.22 */
class FuzzingL3PagingRequestType1 : public L3RRMessage {

	private:

	std::vector<FuzzingL3MobileIdentity> mMobileIDs;
	ChannelType mChannelsNeeded[2];
	bool mBitEnabled;
	...
	FuzzingL3PagingRequestType1()
		:L3RRMessage()
	{
		// The empty paging request is a single untyped mobile ID.
		mMobileIDs.push_back(FuzzingL3MobileIdentity());
		mChannelsNeeded[0]=AnyDCCHType;
		mChannelsNeeded[1]=AnyDCCHType;
	}

	FuzzingL3PagingRequestType1(const FuzzingL3MobileIdentity& wId, ChannelType wType)
		:L3RRMessage(), mBitEnabled(false)
	{
		mMobileIDs.push_back(wId);
		mChannelsNeeded[0]=wType;
		mChannelsNeeded[1]=AnyDCCHType;
	...
	FuzzingL3PagingRequestType1(BitVector * wFuzzingData, ChannelType wType)
		:L3RRMessage(), mBitEnabled(true), mFuzzingData(wFuzzingData)
	{
		//mMobileIDs.push_back(wId);
		mChannelsNeeded[0]=wType;
		mChannelsNeeded[1]=AnyDCCHType;
	...
	FuzzingL3PagingRequestType1(const FuzzingL3MobileIdentity& wId1, ChannelType wType1,
			const FuzzingL3MobileIdentity& wId2, ChannelType wType2)
		:L3RRMessage()
	{
		mMobileIDs.push_back(wId1);
		mChannelsNeeded[0]=wType1;
...
class L3SystemInformationType3 : public L3RRMessage {
	...
	size_t bodyLength() const { return 16+4;}
	void writeBody(L3Frame &dest, size_t &wp) const;
	void text(std::ostream&) const;
};
...
class L3SystemInformationType4 : public L3RRMessage {
	...
	{
		return mLAI.lengthV() +
		mCellSelectionParameters.lengthV() + mRACHControlParameters.lengthV()+10;
	}
