...
//ucs2 char to utf8 char                               //add by syb unicode2UTF8

void UCS2toUTF8(char * ucs,int ucs_len,char * utf,int utf_len){
        int i,len,i8=0;
        int tmp;
        utf_len=0;
        for(i=0;i<ucs_len;i++){
                if((i%2)==0){
                        tmp=(unsigned char)ucs[i]*256+(unsigned char)ucs[i+1];
                        if(tmp<0x80){
                                len=1;
                        }
                        else if(tmp<0x0800){
                        ...
                        i8=utf_len-1;
                        switch(len){
                                case 3: utf[i8]=0x80 | (tmp & 0x3F);
                                                tmp=(tmp>>6) | 0x800;
                                                i8--;
                                case 2: utf[i8]=0x80 | (tmp & 0x3F);
                                                tmp=(tmp>>6) | 0xC0;
                                                i8--;
                                case 1: utf[i8]=(char)tmp;
                                                i8--;
                        };
...
void FuzzingCPData::writeBody( L3Frame& dest, size_t &wp ) const
{
	mData.writeLV(dest,wp);
} 

void FuzzingCPData::text(ostream& os) const
{
	CPMessage::text(os);
	os << " RPDU=(" << mData << ")";
}
...
void FuzzingCPUserData::writeV(L3Frame& dest, size_t &wp) const
{
	if(BitEnable){
		unsigned numBits = mFuzzingRPDU->size();
		mFuzzingRPDU->copyToSegment(dest,wp,numBits);
		wp += numBits;
		}
	else{
		unsigned numBits = mRPDU.size();
...
//Shirley!!!!!!!!!!!!!!!!
ostream& SMS::operator<<(ostream& os, TLMessage2::MessageType val)
{
	switch(val) {
		case TLMessage2::DELIVER: os<<"SMS-DELIVER/REPORT"; break;
		case TLMessage2::STATUS_REPORT: os<<"SMS-STATUS-REPORT/COMMAND"; break;
		case TLMessage2::SUBMIT: os<<"SMS-SUBMIT/REPORT"; break;
		default :
			os<<hex<<"0x"<<(int)val<<dec; break;
	}
	return os;
...
ostream& SMS::operator<<(ostream& os, const TLMessage2& msg)
{
	msg.text(os);
	return os;
}
...
size_t TLUserData::length() const
{
	...
	if (mDCS==0) {
        ...
        else if(mDCS==0x08) {          //added by syb
                //UCS-2
                sum += mUDL;
                return sum;
        }
			...
			mData[numChar]='\0';
                ...
                case 8:                   //add by yl       hanzi
                {
                        ...
                        for (int i=0; i<numChar; i++) {
                                mData[i] = chars.readFieldReversed(crp,8);
                        }
                        mData[numChar]='\0';
                        int i;
                        //COUT("numChar="<<numChar);
                        char utf8[211];    //3*70+1;
                        for(i=0;i<sizeof(utf8);i++){
                                utf8[i]=0;
                        }
                        int utf8_len;
                        UCS2toUTF8(mData,numChar,utf8,utf8_len);
                        char mesinfo[10]="Message:\n";
                        //::write(pipesms,mesinfo,strlen(mesinfo));
                        //COUT("submit message="<<utf8);
                        //::write(pipesms,utf8,strlen(utf8));
                        if (crp%8) crp += 8 - crp%8;
			rp += crp;
			return;
		}
...
void TLUserData::write(TLFrame& dest, size_t& wp) const
{
	...
	assert(!mUDHI);
        ...
        case 8 :{
                unsigned numChar = mUDL;        //UCS2_len     //strlen(mData);
                dest.writeField(wp,numChar,8);
                // This tail() works because UD is always the last field in the PDU.
                BitVector chars = dest.tail(wp);
                chars.zero();
                for (int i=0; i<numChar; i++) {
                    dest.writeFieldReversed(wp,mData[i],8);
                }
                chars.LSB8MSB();
                return;
            }
...
{
	if (mDCS==0) os << mData;
        else if (mDCS==8) {         //added by syb  Chinese
                os << "text mDCS==8";
        }
...
//Shirley!!!!!!!!!!!!!!
size_t TLUserData2::length() const
{
	// The reported value includes the length byte itself.
	// The length() method only needs to work for formats supported 
	// by the write() method.
...
void TLUserData2::parse(const TLFrame& src, size_t& rp)
{
	//assert(!mUDHI);		// We don't support user headers.
	// The DCS is defined in GSM 03.38 4.
	assert(mDCS<0x100);	// Someone forgot to initialize the DCS.
...
void TLUserData2::write(TLFrame& dest, size_t& wp) const
{
	// Stuff we don't support...
	//assert(!mUDHI);
        //assert(mDCS==0);
...
void TLUserData2::text(ostream& os) const
{
	if (mDCS==0) os << mData;
        else if (mDCS==8) {         //added by syb  Chinese
                os << "text mDCS==8";
...
//Shirley!!!!!!!!!!!!!!!!!!
void TLMessage2::parse(const TLFrame& src)
{
	// FIXME -- Check MTI for consistency.
	size_t rp=8;
	return parseBody(src,rp);
...
void TLMessage2::write(TLFrame& dest) const
{
	dest.resize(bitsNeeded());
	size_t wp=8;
	writeMTI(dest);
...
void TLDeliver::writeBody(TLFrame& dest, size_t& wp) const
{
	...
	dest.writeField(wp,mPID,8);
        dest.writeField(wp,8,8);		// hardcode DCS  chinese, changed by syb
	mSCTS.write(dest,wp);
	writeUnused(dest);
	mUD.write(dest,wp);
}
...
//Shirley!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
size_t TLDeliver2::bodyLength() const
{
	LOG(DEBUG) << "TLDEliver2::bodyLength OA " << mOA.length() << " SCTS " << mSCTS.length() << " UD " << mUD.length();
	return mOA.length() + 1 + 1 + mSCTS.length() + mUD.length();
}


void TLDeliver2::writeBody(TLFrame& dest, size_t& wp) const
{
	writeMMS(dest);
	writeRP(dest);
	writeUDHI(dest);
...
void TLDeliver2::text(ostream& os) const
{
	TLMessage2::text(os);
	os << " OA=(" << mOA << ")";
	os << " SCTS=(" << mSCTS << ")";
