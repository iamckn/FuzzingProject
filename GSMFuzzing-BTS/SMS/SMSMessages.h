...
using namespace std;           //add by syb
using namespace GSM;
using namespace SMS;
...
class TLUserData : public TLElement {
	...
	unsigned mUDL;		///< User Data Length, added by syb
	...
	TLUserData(const char* text, unsigned wUDL, bool wUDHI=false)
		:TLElement(),
		mDCS(8),
		mUDL(wUDL),
		mUDHI(wUDHI)
	//{ strncpy(mData,text,sizeof(mData)-1); mData[sizeof(mData)-1]='\0'; }
	{ memcpy(mData,text,sizeof(mData)-1); mData[sizeof(mData)-1]='\0'; }
...
//Shirley!!!!!!!!!!!!!!!!!!
/** GSM 03.40 9.2.3.24 */
class TLUserData2 : public TLElement {
	...
	/** Initialize the DCS with a non-valid value. */
	TLUserData2(unsigned wDCS=0x100, bool wUDHI=true) //Shirley!!!!!!!!!!!!!!
		:TLElement(),
		mDCS(wDCS),
		mUDHI(wUDHI)
	{ mData[0]='\0'; }
	...
	/** Initialze from a simple C string. */
	TLUserData2(const char* text, unsigned wUDL, bool wUDHI=true) //Shirley!!!!!!!!!!!!!!
		:TLElement(),
		mDCS(8),
		mUDL(wUDL),
		mUDHI(wUDHI)
...
class TLMessage {
	...
	*/
	...
	TLMessage()
		:mMMS(false),mSRI(false),mUDHI(false),mRP(false)
	{}
...
class TLMessage2 {
	...
	*/
	...
	TLMessage2()
		:mMMS(false),mSRI(false),mUDHI(true),mRP(false)  //Shirley!!!!!!!!!!!!!!!!!
	{}
...
//Shirley!!!!!!!!!!!!!!!!!!!!!!!
std::ostream& operator<<(std::ostream& os, TLMessage2::MessageType val);
std::ostream& operator<<(std::ostream& os, const TLMessage2& msg);
...
class TLSubmit : public TLMessage {
	...
	const char * destination() { return mDA.digits();} //added by syb
...
/** GSM 03.40 9.2.2.1, downlink */
class TLDeliver2 : public TLMessage2 {
	...
	// Hardcode DCS.
	TLTimestamp mSCTS;		///< service center timestamp, GSM 03.40 9.2.3.11
	TLUserData2 mUD;			///< user data

	public:

	TLDeliver2(const TLAddress& wOA, const TLUserData2& wUD, unsigned wPID=0)
		:TLMessage2(),
		mOA(wOA),mPID(wPID),mUD(wUD)
	{ }

	TLDeliver2(const TLUserData2& wUD)
		:TLMessage2(),
		mUD(wUD)
	{}
...
//TLMessage2* parseTL( const TLFrame& frame );//???????????????????????????????
...
class RPAddress : public GSM::L3CalledPartyBCDNumber {
	...
	RPAddress(TypeOfNumber wType, const char* wDigits)
		:L3CalledPartyBCDNumber(wType,wDigits)
	{}
...
//Shirey!!!!!!!!!!!!!!
	RPUserData(const TLMessage2& TLM)
		:L3ProtocolElement()
	{
		TLM.write(mTPDU);
	}
...
class RPData : public RPMessage {
	...
	{}
//Shirley!!!!!!!!!!!!!!!!!!!!!!
	RPData(unsigned ref, const RPAddress& wOriginator, const TLMessage2& TLM)
		:RPMessage(ref),
		mOriginator(wOriginator),mUserData(TLM)
	{}

	RPData(unsigned ref, const RPAddress& wOriginator, const RPUserData& RPU)
		:RPMessage(ref),
		mOriginator(wOriginator),mUserData(RPU)
	{}


	RPData(unsigned ref, const TLMessage& TLM)
		:RPMessage(ref),
		mUserData(TLM)
	{}
//Shirley!!!!!!!!!!!!!!!!!!!!!!	
	RPData(unsigned ref, const TLMessage2& TLM)
		:RPMessage(ref),
		mUserData(TLM)
	{}
	...
	const char * originator(){return mOriginator.digits();} //added by syb
	const char * destination(){ return mDestination.digits();}
...
/** GSM 04.11 7.3.1 */
class FuzzingRPData : public RPMessage {
	...
	FuzzingRPData():RPMessage() {}

	/** This is a constructor for the downlink version of the message. */
	FuzzingRPData(unsigned ref, const RPAddress& wOriginator, const TLMessage& TLM)
		:RPMessage(ref),
		mOriginator(wOriginator),mUserData(TLM)
	{}

	FuzzingRPData(unsigned ref, const TLMessage& TLM)
		:RPMessage(ref),
		mUserData(TLM)
	{}
...
/** GSM 04.11 8.1.4.1 */
class FuzzingCPUserData : public GSM::L3ProtocolElement {

	private:
		
		RLFrame mRPDU;		///< relay-layer protocol data unit
		bool BitEnable;
		BitVector *mFuzzingRPDU;		///< relay-layer protocol data unit

	public:

	FuzzingCPUserData()
		:L3ProtocolElement(), BitEnable(false)
	{}

	FuzzingCPUserData(BitVector * RPM)
		:L3ProtocolElement(), BitEnable(true),
		mFuzzingRPDU(RPM)
	{
	
	}

	FuzzingCPUserData(const RPMessage& RPM)
		:L3ProtocolElement(), BitEnable(false),
		mRPDU(RPM.bitsNeeded())
	{
		RPM.write(mRPDU);
	...
	size_t lengthV() const { if(BitEnable) return mFuzzingRPDU->size()/8; else return mRPDU.size()/8;  }
	void writeV(GSM::L3Frame& dest, size_t &wp) const;
	void parseV(const GSM::L3Frame& src, size_t &rp, size_t expectedLength){}
	void parseV(const GSM::L3Frame& src, size_t &rp) { assert(0); }
	void text(std::ostream& os) const { if(BitEnable) mFuzzingRPDU->hex(os); else  mRPDU.hex(os);}
};
...
/** GSM 04.11 7.2.1 */
class FuzzingCPData : public CPMessage
{
	private:

	FuzzingCPUserData mData;

	public:	


	FuzzingCPData(unsigned wTIFlag, unsigned wTIValue, BitVector * wFuzzingBit)
		:CPMessage(wTIFlag, wTIValue),
		mData(wFuzzingBit)
 	{ }
