...
GSMConfig gBTS;
FuzzingControl gFuzzingControl(gConfig.getNum("Fuzzing.Port"),gConfig.getStr("Fuzzing.IP"));
char gFuzzingIMSI[16];
...
int main(int argc, char *argv[])
{
	...
	memset(gFuzzingIMSI,0,sizeof(gFuzzingIMSI));
	...
	*/
	...
	while (1) {
		...
		// The parser returns -1 on exit.
		if (gParser.process(inbuf,cout,cin)<0) {cout<<"error 3"; break;}
#endif // !HAVE_LIBREADLINE ]
	}
