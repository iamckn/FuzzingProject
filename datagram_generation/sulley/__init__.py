...
def s_GSMblock_start (name, group=None, encoder=None, dep=None, dep_value=None, dep_values=[], dep_compare="=="):
    '''
    Open a new block under the current request. This routine always returns True so you can make your fuzzer pretty
    with indenting::
    ...
    block = blocks.GSMblock(name, blocks.CURRENT, group, encoder, dep, dep_value, dep_values, dep_compare)
    blocks.CURRENT.push(block)
...
def s_GSMbit_field( value, width,min_repeat=1,max_repeat=10,step=None,max_num=None, endian="<", format="binary", signed=False, full_range=False, fuzzable=True, name=None):

    
    GSMbit_field = primitives.GSMrepeat_bit_field(value, width,min_repeat,max_repeat,step,max_num, endian, format, signed, full_range, fuzzable, name)
    blocks.CURRENT.push(GSMbit_field)
