...
class string (base_primitive):
    ...
    def add_long_strings (self, sequence):
        ...
        for length in [32,64,128, 255, 256, 257, 511, 512, 513, 1023, 1024, 2048, 2049, 4095, 4096, 4097, 5000, 10000, 20000,
...
class bit_field (base_primitive):
    ...
    def add_integer_boundaries (self, integer):
        ...
        for i in xrange(-bit_field.bit_minEdge, bit_field.bit_minEdge):
            case = integer + i
...
class GSMrepeat_bit_field(bit_field):
    def __init__ (self, value, width,min_repeat=1,max_repeat=10,step=None,max_num=None, endian="<", format="binary", signed=False, full_range=False, fuzzable=True, name=None):
        bit_field.__init__ (self,value, width, None, endian, format, signed, full_range, fuzzable, name)
        self.min_repeat=min_repeat
        self.max_repeat=max_repeat
        self.max_num=max_num
        self.step=step
        self.repeat=min_repeat
        if step:
            self.mutate_index2=min_repeat
        else:
            self.mutate_index2=0
    
        if not self.step and not self.max_num:
            self.step=1
            
    def mutate(self):
        if self.step:
            self.repeat=self.mutate_index2
            self.mutate_index2+=self.step
            if self.mutate_index2<=self.max_repeat:
                return True
            else:
                self.mutate_index2=self.min_repeat
                return bit_field.mutate(self)
        elif self.max_num:
            self.repeat=random.randint(self.min_repeat,self.max_repeat)
            self.mutate_index2+=1
            if self.mutate_index2<=self.max_num:
                return True
            else:
                self.mutate_index2=0
                return bit_field.mutate(self)
    ...
    def render(self):
        ...
        for i in xrange(self.repeat):
            self.value=(self.value<<t_width)+t_value
        self.rendered = bit_field.render(self)
        self.value=t_value
        self.width=t_width
        return self.rendered
